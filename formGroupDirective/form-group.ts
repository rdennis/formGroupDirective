...
m.directive('formGroup', ['formGroupConfig', function (defaultConfig: IFormGroupConfig) {
   var definition: ng.IDirective = {
      ...
      compile: function (element, attrs) {
         ...
         return function link($scope, element: ng.IAugmentedJQuery, attrs: ng.IAttributes) {
            ...
            if ($scope.invalidOnBlur) {
               var setDirty = function () { inputCtrl.$dirty = true }
            ...
            $scope.$watch(
               function () {
                  if (inputCtrl.$invalid && (inputCtrl.$dirty || formCtrl.$submitted)) return 'invalid'
